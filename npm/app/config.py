...
def load_settings(
    ...
    task_path = Path(task_config_path_override or os.getenv("TASK_CONFIG", "config.json")).expanduser()
    artifacts_dir = Path(os.getenv("ARTIFACTS_DIR", "artifacts")).expanduser()
    default_state = Path("storage-state.json")
    # 优先使用显式传入的登录态路径（多租户隔离），其次环境变量，最后默认文件
    state_val = storage_state_path or _optional_env("DOUYIN_STORAGE_STATE")
    ...
    return Settings(
        ...
        storage_state=state_val or (str(default_state) if default_state.is_file() else None),
        cookie=_optional_env("DOUYIN_COOKIE"),
        headless=_parse_bool(os.getenv("HEADLESS", "true"), "HEADLESS"),
...
def load_task(settings: Settings) -> TaskConfig:
    ...
    target_open_retries = raw.get("target_open_retries", 5)
